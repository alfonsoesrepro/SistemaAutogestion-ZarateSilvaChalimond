...
documentacion/
  Documentaci贸n_Florencia_Zarate.md
  Documentaci贸n_Cesar_Chalimond.md
  Documentaci贸n_Alfonso_Silva.md
```
...
- **Primer proyecto grupal con Git:** al ser el primer proyecto que hacemos en
  ...
  gener贸 confusion al principio hasta que entendimos el flujo completo de
  trabajo con Git
...
## ?? Segunda Instancia Evaluativa (IE2): Interfaz Gráfica y Persistencia

En esta segunda etapa, el sistema fue migrado de consola a una aplicación de escritorio con interfaz gráfica, aplicando la arquitectura **MVC + DAO** y persistiendo los datos.

### Instrucciones de Ejecución (Actualización IE2)
1. Abrir el proyecto con **Apache NetBeans**.
2. Requiere **JDK 17** o superior.
3. **Dependencia crucial:** Para que el Bonus JDBC funcione, es necesario a?adir el driver `mysql-connector-java` (o equivalente) en las librerías del proyecto.
4. Configurar la base de datos siguiendo las instrucciones de la sección "Configuración de Base de Datos (Bonus JDBC)".
5. Ejecutar la clase principal que lanza el primer `JFrame` de la vista.

### Arquitectura del Proyecto (MVC + DAO)
El proyecto se dividió estrictamente en cuatro paquetes:
* **modelo:** Clases de la IE1 reutilizadas (`Estudiante`, `Materia`, etc.) con la lógica de negocio intacta.
* **vista:** Interfaces gráficas dise?adas en Swing. No contienen lógica de negocio ni manipulan datos directamente; solo capturan eventos y los envían al Controlador.
* **controlador:** Puente orquestador. Recibe acciones de la Vista, procesa con el Modelo y delega el guardado al DAO. Está libre de librerías Swing.
...
| :--- | :--- |
| **Cesar Nicolas Chalimond** | **Controlador y Persistencia JDBC (Bonus):** Desarrollo de la lógica mediadora aislando la interfaz del modelo. Creación de la base de datos MySQL y programación de los DAOs exclusivos mediante `Connection`, `PreparedStatement` y `ResultSet`. |
| **Florencia Agustina Zarate** | *[Completar: Ej. Dise?o UI/UX en Figma y maquetado Swing en NetBeans]* |
| **Alfonso Silva** | *[Completar: Ej. Implementación de persistencia en archivos de texto, ajustes de serialización/toTexto]* |

### ??? Configuración de Base de Datos (Bonus JDBC)
Para evaluar la implementación a base de datos, ejecutar el siguiente script en MySQL antes de iniciar la aplicación. Esto generará la estructura requerida por los DAOs:
